...
export const geminiService = {
  ...
  async analyzeMemory(
    ...
    if (isMock || !apiKey) {
      ...
      let tags = ["가족", "추억"];
      let summary =
        "이 기록은 우리 가족이 함께 보낸 따스한 순간을 생생하게 보관하고 있습니다.";
      ...
      if (
        ...
        tags = ["가족여행", "나들이", "힐링"];
        summary =
          "일상에서 벗어나 온 가족이 함께 모여 행복한 자유와 미소를 나누었던 특별한 여행의 기억입니다.";
      } else if (
        text.includes("생일") ||
        text.includes("축하") ||
        text.includes("파티")
        ...
        tags = ["생일잔치", "기념일", "가족모임"];
        summary =
          "소중한 가족 구성원의 탄생이나 특별한 성장을 축하하며, 서로를 향한 아낌없는 축복을 나누던 밤입니다.";
      } else if (
        text.includes("아프") ||
        text.includes("병원") ||
        text.includes("힘들")
        ...
        tags = ["위로", "극복", "건강"];
        summary =
          "고난이나 아픔 앞에서도 한마음으로 손을 맞잡고 위로하며 돈독한 가족애로 극복해 나가던 순간입니다.";
      } else if (
        text.includes("요리") ||
        text.includes("음식") ||
        text.includes("식사") ||
        ...
        tags = ["집밥", "가족식사", "소소한 일상"];
        summary =
          "어머니나 아버지의 따스한 정성이 깃든 한 끼 식사를 함께 나누며 도란도란 정담을 속삭이던 평화로운 일상입니다.";
      }
    ...
    try {
      ...
      console.error(
        "Gemini API Memory analysis failed, fallback to mock:",
        err,
      );
  ...
  async generateDocumentary(
    ...
    if (isMock || !apiKey) {
      ...
      const years = Array.from(
        new Set(sortedMemories.map((m) => new Date(m.date).getFullYear())),
      );
      if (years.length === 0) {
      ...
      const chapters: DocumentaryChapter[] = years.map((yr, idx) => {
        const yearMems = sortedMemories.filter(
          (m) => new Date(m.date).getFullYear() === yr,
        );
        return {
          id: `chap_${yr}_${idx}`,
          year: yr,
      ...
      return {
        ...
        introduction:
          "이 기록은 우리 가족이 수년에 걸쳐 함께 걸어온 발자취이며, 서로를 꼭 보듬던 소중한 대화와 눈부신 찰나의 순간들을 모아 엮은 진솔한 우리의 다큐멘터리입니다.",
        chapters,
        conclusion:
          "시간은 흐르고 계절은 바뀌어도 우리가 쌓아 올린 사랑의 깊이는 영원히 우리 가문의 마음에 남아 밝은 등불이 될 것입니다.",
    ...
    try {
      ...
      const chaptersWithMedia: DocumentaryChapter[] = (
        parsed.chapters || []
      ).map((ch: any) => {
        const yearMems = sortedMemories.filter(
          (m) => new Date(m.date).getFullYear() === ch.year,
        );
        return {
          ...ch,
          memories: yearMems.map((m) => ({
      ...
      console.error(
        "Gemini API Documentary generation failed, fallback to mock:",
        err,
      );
  ...
  async askInterviewQuestion(
    ...
    try {
      ...
      console.error(
        "Gemini API Interview question failed, fallback to mock:",
        err,
      );
  ...
  async chatWithFamilyPersona(
    ...
    if (isMock || !apiKey) {
      ...
      if (customPersona) {
        ...
        const kakaoMem = memories.find(
          (m) => m.title.includes("카카오톡") && m.description,
        );
        let foundKakaoReply = "";
        if (kakaoMem) {
          ...
          const userIdx = lines.findIndex((l) => {
            ...
            return (
              low.includes(`${userRole || "승환"}:${normalizedMsg}`) ||
          ...
          if (userIdx !== -1) {
            ...
            for (let i = userIdx + 1; i < lines.length; i++) {
              ...
              if (currentLine.startsWith(`${customPersona.name}:`)) {
                replies.push(
                  currentLine.substring(customPersona.name.length + 1).trim(),
                );
              } else if (
                currentLine.includes(":") &&
                !currentLine.startsWith(`${customPersona.name}:`)
              ) {
            ...
            if (replies.length > 0) {
              ...
              const nicknameSubject =
                nickname.endsWith("이") ||
                nickname.charCodeAt(nickname.length - 1) % 2 === 0
                  ? nickname
                  : `${nickname}이`;
        ...
        if (foundKakaoReply) {
          ...
          cleanMsg.includes("좋아하는 사람") ||
          cleanMsg.includes("누가 좋아") ||
          cleanMsg.includes("누굴 좋아")
        ) {
        ...
        if (cleanMsg.includes("밥") || cleanMsg.includes("식사")) {
          responseText =
            "어라, 밥은 제때 먹고 다니는 거지? 사내든 자식이든 끼니를 거르면 힘을 못 쓴단다. 아빠가 항상 네 건강 챙기라고 말하는 거 알지? 고기 꼭 챙겨 먹어라.";
        } else if (
          cleanMsg.includes("힘들") ||
          cleanMsg.includes("지쳐") ||
          cleanMsg.includes("우울")
          ...
          responseText =
            "어깨가 많이 무겁겠구나. 아빠도 그 시절엔 다 그랬단다. 하지만 너는 나보다 훨씬 영리하고 단단한 아이니까 이번 고비도 현명하게 넘길 거야. 아빠가 든든하게 받쳐 줄 테니 겁내지 마라.";
        } else if (cleanMsg.includes("사랑") || cleanMsg.includes("고마워")) {
          responseText =
            "허허, 갑자기 고맙다니 쑥스럽구나. 아빠도 항상 우리 자식 자랑스럽게 생각하고 아주 사랑한단다. 네가 우리 집에 태어나 준 게 아빠 인생 최고의 선물이야.";
        } else {
          responseText =
            "아빠다. 우리 아들/딸, 요즘 하려는 일들은 잘 가닥이 잡혀가고 있니? 세상이 가끔 숨차게 굴어도, 내 자식 기죽지 말고 기개 있게 나가라. 아빠는 늘 네 편이다.";
        }
      } else if (personaId === "grandma") {
        if (cleanMsg.includes("밥") || cleanMsg.includes("식사")) {
          responseText =
            "아이고, 밥 얘기 나오니까 할미가 가슴이 아프네. 일하느라 굶고 다니는 거 아니제? 할미가 맛난 거 잔뜩 해놓고 기둘리고 있으니까네 얼릉 와서 두 그릇 뚝딱 하그라. 내 새끼 튼튼해야제.";
        } else if (
          cleanMsg.includes("힘들") ||
          cleanMsg.includes("지쳐") ||
          cleanMsg.includes("우울")
          ...
          responseText =
            "오메, 누가 우리 이쁜 강아지를 힘들게 하더냐! 세상 일 다 부질없고, 우리 새끼 마음 편한 기 최고제. 할미가 꼭 안아줄텡게 훌훌 털어내그라. 다 지나가고 해뜰 날 온다잉.";
        } else if (cleanMsg.includes("사랑") || cleanMsg.includes("고마워")) {
          responseText =
            "아이고 참말이가! 할미는 우리 강아지 말만 들어도 배가 부르고 눈물이 다 날라 칸다. 고맙고 이쁜 내 강아지, 할미가 하늘만큼 땅만큼 최고로 아끼고 사랑한데이.";
        }
      } else {
        if (cleanMsg.includes("밥") || cleanMsg.includes("식사")) {
          responseText =
            "우리 아들/딸, 밥은 먹고 일하고 있니? 늘 건강이 제일 중요하단다. 바빠도 끼니 거르지 말고 꼭 따뜻한 밥 챙겨 먹으렴. 엄마는 늘 네 걱정뿐이야.";
        } else if (
          cleanMsg.includes("힘들") ||
          cleanMsg.includes("지쳐") ||
          cleanMsg.includes("우울")
          ...
          responseText =
            "어이쿠, 힘든 일이 있었나 보구나. 네 마음 다치지 않게 엄마가 늘 여기서 기도하고 있단다. 인생이라는 게 파도가 치기도 하지만 우리 아이는 마음이 단단해서 지혜롭게 헤쳐 나갈 수 있어. 힘내렴, 사랑해.";
        } else if (cleanMsg.includes("사랑") || cleanMsg.includes("고마워")) {
          responseText =
            "엄마도 우리 자식 너무 사랑하고 늘 고마워. 네가 태어나 주던 그날부터 지금까지, 넌 늘 우리 가문의 가장 큰 보석이자 기쁨이었단다. 내 보물, 오늘도 행복 가득한 하루 되렴.";
        } else {
          responseText =
            "엄마야. 우리 착한 아이, 오늘 하루는 어땠는지 묻고 싶네. 힘든 일은 없었는지, 네 조그마한 일상이 늘 궁금하고 소중하단다.";
        }
      }
    ...
    try {
      ...
      const memoryContext = memories
        .map(
          (m) =>
            `- [${m.type}] 제목: ${m.title}, 설명: ${m.description || ""}, 감정: ${m.aiAnalysis?.sentiment || ""}`,
        )
        .join("\n");
      const interviewContext = interviews
        .map((i) => `- 질문: ${i.questionText}, 대답: ${i.responseText}`)
        .join("\n");

      let personaName = customPersona
        ? `${customPersona.name} (${customPersona.relationship})`
        : "엄마 (어머니)";
      let defaultPersonaTone = customPersona
        ? customPersona.description
        : "세상에서 가장 다사롭고 자상하게 자녀의 건강과 끼니를 살뜰히 보살피는 말투";

      if (!customPersona) {
        if (personaId === "dad") {
          personaName = "아빠 (아버지)";
          defaultPersonaTone =
            "묵묵하고 든든하지만 속 깊이 자식을 아끼며 격려하는 어투";
        } else if (personaId === "grandma") {
          personaName = "할머니";
          defaultPersonaTone =
            '"우리 귀한 똥강아지"라고 부르는 등, 지극한 내리사랑 and 인자하며 구수한 사투리가 가미된 친숙한 말투';
        } else if (personaId === "grandpa") {
          personaName = "할아버지";
          defaultPersonaTone =
            "차분하고 지혜로우며 손주를 끔찍이 아끼는 넉넉하고 격조 높은 어른의 말투";
        } else if (personaId === "spouse") {
          personaName = "배우자 (아내/남편)";
          defaultPersonaTone =
            "평생을 함께하는 인생의 반려자이자 동반자로서, 장난기도 섞여 있고 격려와 사랑이 넘쳐나는 친근한 말투";
        }
      }
      ...
      const systemPrompt = `
        ...
        5. 절대 자신이 인공지능(AI)이나 대규모 언어 모델이라는 사실을 밝히지 마십시오.
        6. 사용자가 방금 보낸 말에만 자연스럽게 답하고, 이전 대화는 흐름을 맞추는 데 필요한 만큼만 참고하십시오.
      `;
      ...
      const conversationContext = chatHistory
        ...
        .map((msg) => {
          const speaker =
            msg.sender === "user" ? userRole || "사용자" : personaName;
      ...
      const prompt = `${systemPrompt}
        ...
        [최근 대화]
        ${conversationContext || "아직 대화가 시작되지 않았습니다."}

        [사용자의 현재 말]
        ${lastUserMessage}

        [출력 지침]
        ${personaName}의 실제 말투로 한 답변만 출력하십시오.
      `;
      ...
      console.error(
        "Gemini API Family persona chat failed, fallback to mock:",
        err,
      );
      return this.chatWithFamilyPersona(
        chatHistory,
        memories,
        interviews,
        personaId,
  ...
  async generateGuideAnswer(
    ...
    try {
      const endpoint = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`;
      const prompt =
        "당신은 컴퓨터와 스마트폰 조작이 다소 서투신 분들을 위해 쉽고 친절하게 길을 안내하는 '담아' 앱의 다정다감한 가이드 도우미 비서 '도이'입니다.\n\n" +
      ...
      console.error(
        "Gemini API Guide generation failed, fallback to rules:",
        err,
      );

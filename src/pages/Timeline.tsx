...
import { Clock, Filter, Trash2, Edit3, Image, Video, Mic, FileText, Search, Heart, AlertCircle, X, Calendar, FileText as DescIcon, Check, Loader2 } from 'lucide-react';
...
export const Timeline: React.FC = () => {
  ...
  const availableYears = useMemo(() => {
    const years = memories.map(m => new Date(m.date).getFullYear().toString());
    ...
    return memories
      .filter(item => {
        ...
        const matchesSearch = searchQuery.trim() === '' || 
          ...
          item.aiAnalysis?.tags?.some(tag => tag.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const handleAnalyze = async (item: MemoryItem) => {
    try {
      ...
      updateMemory(item.id, { aiAnalysis: { ...result, analyzedAt: new Date().toISOString() } });
    } catch (err) {
      console.error(err);
  ...
  const getTypeIcon = (type: MemoryType) => {
    switch (type) {
      case 'photo': return { icon: Image, color: 'text-apple-blue bg-apple-blue/10' };
      case 'video': return { icon: Video, color: 'text-brand-primary bg-brand-light' };
      case 'audio': return { icon: Mic, color: 'text-apple-green bg-apple-green/10' };
      case 'memo': return { icon: FileText, color: 'text-brand-accent bg-brand-accent/10' };
    }
  };
  ...
  return (
    <div className="space-y-8 animate-float-up max-w-4xl mx-auto">
      {/* Page Title */}
      <div>
        <h2 className="text-3xl font-bold tracking-tight text-apple-gray-800">추억 타임라인</h2>
      ...
      {filteredSortedMemories.length > 0 ? (
        <div className="relative pl-6 md:pl-28 space-y-12">
          ...
          {filteredSortedMemories.map((item) => {
            ...
            return (
              <div key={item.id} className="relative flex flex-col md:flex-row gap-6 animate-fade-in z-10 group">
                ...
                <div className="hidden md:block absolute -left-28 top-3 text-right w-20">
                  <span className="block text-lg font-extrabold text-apple-gray-800 leading-none">{displayYear}</span>
                  <span className="block text-[11px] font-bold text-apple-gray-300 mt-1 uppercase tracking-wider">{displayMonthDay}</span>
                </div>

                {/* Node Ring Icon */}
                <div className={`absolute left-0 md:left-20 w-8 h-8 rounded-full border-2 border-white flex items-center justify-center shadow-md transform -translate-x-[15px] md:-translate-x-0 ${iconColor} z-20`}>
                ...
                <div className="flex-1 bg-white rounded-3xl p-5 md:p-6 border border-apple-gray-100/40 shadow-apple-card hover:shadow-apple-card-hover group-hover:scale-[1.01] transition-all duration-500 space-y-4">
                    ...
                    <div className="rounded-2xl overflow-hidden border border-apple-gray-100 max-h-80 shadow-inner bg-apple-gray-50 flex items-center justify-center">
                      ...
                      {item.type === 'video' && (
                        <div className="relative w-full aspect-video flex items-center justify-center bg-black group-media rounded-2xl overflow-hidden">
                          {item.mediaUrl && !item.mediaUrl.includes('unsplash.com') ? (
                            <video src={item.mediaUrl} controls className="w-full h-full object-contain" playsInline />
                          ) : (
                            <>
                              <img src={item.mediaUrl || 'https://images.unsplash.com/photo-1543269865-cbf427effbad?w=800'} alt="Video thumbnail" className="absolute inset-0 w-full h-full object-cover opacity-60" />
                      ...
                      {item.type === 'audio' && (
                        <div className="w-full p-4 flex items-center gap-3">
                          ...
                          <audio src={item.mediaUrl} controls className="flex-1 h-9 rounded-lg" />
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {item.aiAnalysis ? (
                    <div className="pt-4 border-t border-apple-gray-100 flex flex-col gap-2.5 bg-apple-blue/[0.01]">
                      <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                        ...
                        {item.aiAnalysis.tags?.map((tag, i) => (
                          <span key={i} className="text-[9px] bg-apple-gray-50 border border-apple-gray-100 px-2 py-0.5 rounded-full text-apple-gray-300 font-bold">
                            #{tag}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="pt-3 border-t border-apple-gray-100 flex items-center justify-between">
                      <span className="text-[11px] text-apple-gray-300 font-semibold">아직 깊이를 들여다보지 않은 고요한 기록입니다.</span>
                      <button
        ...
        <div className="bg-white rounded-3xl p-12 text-center border border-apple-gray-100/40 shadow-apple-card max-w-lg mx-auto space-y-4 animate-fade-in">
          ...
          <div>
            <h3 className="text-lg font-bold text-apple-gray-800">해당 조건의 추억이 없습니다</h3>
            <p className="text-xs text-apple-gray-300 mt-1 max-w-xs mx-auto">
              필터 조건을 변경하거나, 우측 상단의 [추억 저장] 탭을 눌러 가족의 첫 번째 따뜻한 이야기를 기록해 보세요.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-apple-gray-950/40 backdrop-blur-md flex items-center justify-center px-4 z-[9999] animate-fade-in">
          <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-lg shadow-[0_25px_60px_-15px_rgba(0,0,0,0.3)] space-y-5 animate-float-up text-apple-gray-800">
            <div className="flex justify-between items-center pb-2 border-b border-apple-gray-50">
              <h3 className="text-lg font-bold text-apple-gray-800">추억 기록 수정</h3>
              <button
